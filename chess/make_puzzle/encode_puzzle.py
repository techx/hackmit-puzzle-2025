# chess/make_puzzle/encode_puzzle.py
import math
...
""".strip()
PARTIAL_GAME_LENGTH = 368
FULL_GAME_LENGTH = 507

PLAINTEXT1 = "Complete a full wikipedia-GIF knight's tour. If a white piece must get out of the way of the tour, move it forward. The black king always prefers moving to squares that are the smallest possible value (a8=0, b8=1, ...), but will never capture a piece. Finish with a mate in 3."
PLAINTEXT2 = "Brilliant move! The flag is as follows: "

assert(len(PLAINTEXT1) == math.floor(PARTIAL_GAME_LENGTH*6/8))
assert(len(PLAINTEXT1) + len(PLAINTEXT2) + 64 == math.floor(FULL_GAME_LENGTH*6/8))
...
def get_ciphertext(flag: str) -> str:
    ...
    PLAINTEXT  = PLAINTEXT1 + PLAINTEXT2 + flag
    assert(len(PLAINTEXT) == math.floor(FULL_GAME_LENGTH*6/8))
