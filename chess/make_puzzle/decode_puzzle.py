...
# ────────────────────────────────────────────────────────────────
CIPHERTEXT_ENCODING = "base64"            # "hex"  or  "base64"
...
# ---------- main -------------------------------------------------------------
def solve(pgn: str, ciphertext: str, ct_enc: str = "hex") -> str:
    key = key_from_pgn(pgn)
    cipher_bytes = (
        bytes.fromhex(ciphertext)
        if ct_enc.lower() == "hex"
        else base64.b64decode(ciphertext)
    )
...
if __name__ == "__main__":
    message = solve(PGN_TEXT, CIPHERTEXT, CIPHERTEXT_ENCODING)
